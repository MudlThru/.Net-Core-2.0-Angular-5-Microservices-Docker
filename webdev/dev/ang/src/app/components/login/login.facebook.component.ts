...
// declare these variables here to let the ts compiler know that they exist
...
export class LoginFacebookComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if(typeof(FB) === 'undefined') {
            //if the FB object is undefined, it means that it's the first time we visit theis page, hence we need to initialize the Facebook SDK
            window.fbAsyncInit = () =>
            //be sure to do this within the local zone, or Angular will be unable to find the local references
            this.zone.run(() => {
                
                FB.init({
                    appId: '532135740460109',
                    xfbml: true,
                    version: 'v2.10'
                });
                ...
                //this will trigger right after the user completes the FB SDK Auth roundtrip successfully
    ...
    //this method will be executed upon the user FB SDKAuth roundtrip completion to create.login the local user
    onConnect(accessToken: string){
        //call TokenController and register/login
        var url = this.baseUrl + "auth/token/facebook";
        var data = {
